...
public class JedisUtil {
    ...
    public boolean setIfAbsent(String key, String value, int cacheSeconds) {
        if (exists(key)) {
            set(key, value, cacheSeconds);
            log.info("redis存key:{}-value:{}-时间:{}", key, value, cacheSeconds);
            return true;
        }
        return false;
    }
    ...
    public long setSet(String key, Set<String> value, int cacheSeconds) {
        ...
        try {
            ...
            result = jedis.sadd(key, (String[]) value.toArray(new String[0]));
            if (cacheSeconds != 0) {
                jedis.expire(key, cacheSeconds);
            }
