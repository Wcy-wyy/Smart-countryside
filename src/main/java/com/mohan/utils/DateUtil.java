...
public class DateUtil {
    ...
    public static Long getBeforeMonth() {
        Date date = new Date();
        return DateUtil.addDays(date, 30).getTime() / 1000;
    }
    ...
    public static Date getTomorrowDateString(Date sDate) {
        sDate = addSeconds(sDate, ONE_DAY_SECONDS);
        return sDate;
    }
    ...
    public static String getMonthDiff(String startDate, String endDate) throws ParseException {
        Date Date1 = DateUtil.parseDateWebFormat(startDate);
        Date Date2 = DateUtil.parseDateWebFormat(endDate);
        String str = "0";
            ...
            if (day <= 0) { //结束日期天 - 开始日期天为负数，则向月借位 ， 借位月 -1 ，天计算为 借位月天数+ 原结束日期天 - 开始日期天
                ...
                if (month < 0) {//借位月减后小于等于0则向年借位
                    ...
                    if (year >= 0) {//如果年大于等于开始日期年，则借位月值为12，否则结束日期小于开始日期，异常
                        month = year * 12 + 11;
                        int monthDay = getDaysByYearMonth(year, month);
                ...
                month = month + year * 12;
                int monthDay = getDaysByYearMonth(year2, month2);
                if (monthDay == day) {
                    month++;
                    day = 0;
                }
    ...
    public static Date getWeekStart() {
        return getWeekStartCal().getTime();
    }
     ...
     * @return
     */
    public static List<Date> getTwoDaysDay(Date dateStart, Date dateEnd) {
        ...
        try {
            ...
            while (calendar.getTime().after(dateStart)) { //倒序时间,顺序after改before其他相应的改动。
                calendar.add(Calendar.DAY_OF_MONTH, -1);
    ...
    public static String date8Str2TimeMinStr(String dateStr){
        LocalDate date=LocalDate.parse(dateStr, DateTimeFormatter.ofPattern("yyyyMMdd"));
//        return date.atStartOfDay().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        return LocalDateTime.of(date, LocalTime.MIN).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
    }
    public static String date8Str2TimeMaxStr(String dateStr){
        LocalDate date=LocalDate.parse(dateStr,DateTimeFormatter.ofPattern("yyyyMMdd"));
        return LocalDateTime.of(date, LocalTime.MAX).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
    }

    public static void main(String[] args) {
        log.info(date8Str2TimeMinStr("20180809"));
        log.info(date8Str2TimeMaxStr("20180809"));
    }
}